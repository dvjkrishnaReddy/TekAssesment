...
class OrderLine {
	...
	public OrderLine(Item item, int quantity) throws Exception {
		...
		assert quantity > 0;
		this.item = item;    //  2. Initializing it properly using this.item 
		this.quantity = quantity; // Initializing it properly using this.quantity 
	}
...
class Order {
	...
	private List<OrderLine> orderLines = new ArrayList<OrderLine>(); // 3. Initializing List object with an ArrayList to overcome Null Pointer Exception
	...
	// 4. Return a new instance of Order 
	public Order clear() {
		return new Order();
	}
}
...
class calculator {
	...
	public static double rounding(double value) {
		return ( Math.round(value * 100.0)) / 100.0; // 5. returning double value instead of int upto two decimal places ex: 12.7667 converts to 12.77
	}
	...
	public void calculate(Map<String, Order> o) {
		...
		for (Map.Entry<String, Order> entry : o.entrySet()) {
			...
			// grandtotal = 0; //6. It is already initialized outside, initializing here makes the globalTotal 0 for every iteration in the loop 
			...
			// 7. Iterating the loop until r.size()-1 because loop starts with 0 
			for (int i = 0; i <= r.size()-1; i++) {
				...
				// 8. Printing out quantity as per the output required
			...
			// total = total + totalTax; //9. Removing it as we need only the total price of items without taxes
...
public class Foo {
	...
	public static void main(String[] args) throws Exception {
		...
		// double grandTotal = 0; // 10. Removing it as it occupies unnecessary memory
		...
		// 11. Emptying the list using c.clear method 
		c = c.clear(); 
		...
		c.add(new OrderLine(new Item("imported bottle of perfume", (float) 27.99), 1)); // Replacing Imported to imported
		...
		c.add(new OrderLine(new Item("box of imported chocolates", (float) 11.25), 1)); // Replacing importd to imported
